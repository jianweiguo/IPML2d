...
def inference(det_net, data_dir):
    ...
    with tf.Session(config=config) as sess:
        ...
        for i, a_img_name in enumerate(imgs):
            ...
            bbx_name = save_dir+'/{}.txt'.format(a_img_name.split('.jpg')[0]);
            #bbx_name = bbx_name.replace('jpg', 'txt')
            #f = open(save_dir+'/res_{}.txt'.format(a_img_name.split('.jpg')[0]), 'w')
            ...
            #for r in res_r:
            #     f.write('{:.3f} {:.1f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.format(det_scores_r_[0], r[0], r[1], r[2], r[3], r[4], r[5]))
            ...
            draw_box_in_img.write_rotate_box_cv(boxes=det_boxes_r_,
                                               labels=det_category_r_,
                                               scores=det_scores_r_,
                                               f_name = bbx_name)
            #save_dir = os.path.join(cfgs.INFERENCE_SAVE_PATH, cfgs.VERSION)
